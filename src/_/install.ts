...
export default ( options: Options ): void => {
    ...
    const { domain, "class": _class, environment, path } = options;
    ...
    if ( environment && !rValidEnvironment.test( environment ) ) {
        throwError( `install environment "${ environment }" is invalid` );
    }
